...
import { Transport, TruckTransport, ShipTransport, AirplaneTransport } from "./01-basic-concept/product.interface";
import { LogisticsCreator, RoadLogistics, SeaLogistics, AirLogistics } from "./01-basic-concept/creator.abstract";
import {
  DocumentExportCreator,
  CsvExportCreator,
  JsonExportCreator,
  PdfExportCreator,
  XmlExportCreator,
} from "./02-advanced-example/document-factory";
...
function runBasicExample(): void {
  ...
  const creators: LogisticsCreator[] = [
    new RoadLogistics(),
    new SeaLogistics(),
    new AirLogistics(),
  ];
  ...
  function selectCreatorBasedOnUrgency(urgency: "standard" | "express" | "economy"): LogisticsCreator {
    switch (urgency) {
      case "express":
        return new AirLogistics(); // Fastest
      case "standard":
        return new RoadLogistics(); // Balanced
      case "economy":
        return new SeaLogistics(); // Cheapest
    }
  }

  const urgencies: Array<"standard" | "express" | "economy"> = ["economy", "standard", "express"];
  ...
  function planDeliveryWithoutPattern(weight: number, type: "truck" | "ship" | "plane"): string {
    ...
    switch (type) {
      ...
      case "ship":
        ...
        break;
      case "plane":
        transport = new AirplaneTransport();
        break;
    }
...
function runAdvancedExample(): void {
  ...
  const exportCreators: { creator: DocumentExportCreator; label: string }[] = [
    { creator: new CsvExportCreator(), label: "CSV" },
    { creator: new JsonExportCreator(), label: "JSON" },
    { creator: new PdfExportCreator(), label: "PDF" },
    { creator: new XmlExportCreator(), label: "XML" },
  ];
  ...
  const jsonCreator = new JsonExportCreator();
  ...
  const dataWithNestedObjects: Record<string, unknown>[] = [
    { name: "Alice", details: { age: 30, city: "NYC" } },
    { name: "Bob", details: { age: 25, city: "LA" } },
  ];

  const csvCreator = new CsvExportCreator();
