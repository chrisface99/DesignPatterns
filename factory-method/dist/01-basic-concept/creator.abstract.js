...
exports.AirLogistics = exports.SeaLogistics = exports.RoadLogistics = exports.LogisticsCreator = void 0;
...
class LogisticsCreator {
     ...
     */
    planDelivery(packageWeight) {
        // Step 1: Call the factory method to get a transport
        // At this point, we DON'T know if it's a truck, ship, or plane
        // We don't NEED to know — the interface is enough
        const transport = this.createTransport();
        // Step 2: Use the transport through its interface
        // The business logic works with ANY transport that implements the interface
        const deliveryMethod = transport.deliver();
        const estimatedTime = transport.estimatedDeliveryTime();
        const totalCost = transport.costPerKg() * packageWeight;
        // Step 3: Compose the result using the product's behavior
        return [
            `📦 Delivery Plan:`,
            `   ${deliveryMethod}`,
            `   ⏱  Estimated time: ${estimatedTime} hours`,
            `   💰 Total cost: $${totalCost.toFixed(2)} (at $${transport.costPerKg()}/kg × ${packageWeight}kg)`,
        ].join("\n");
    }
    /**
     ...
     */
    compareWithStandard(packageWeight) {
        const transport = this.createTransport();
        const cost = transport.costPerKg() * packageWeight;
        // We could even create multiple transports for comparison
        // (though in this basic example, each creator only makes one type)
        return `Shipping ${packageWeight}kg via ${this.getTransportType()}: $${cost.toFixed(2)}`;
    }
}
exports.LogisticsCreator = LogisticsCreator;
/**
 * ============================================================================
...
class RoadLogistics extends LogisticsCreator {
     ...
     */
    createTransport() {
        // This is where the actual object creation happens
        // You could add complex construction logic here:
        // - Reading config
        // - Selecting truck size based on parameters
        // - Applying regional rules
        // - etc.
        return new product_interface_1.TruckTransport();
    }
    getTransportType() {
        return "Road (Truck)";
    }
...
class SeaLogistics extends LogisticsCreator {
    createTransport() {
        return new product_interface_1.ShipTransport();
    }
    getTransportType() {
        return "Sea (Ship)";
    }
...
class AirLogistics extends LogisticsCreator {
    createTransport() {
        return new product_interface_1.AirplaneTransport();
    }
    getTransportType() {
        return "Air (Plane)";
    }
}
exports.AirLogistics = AirLogistics;
/**
 * ============================================================================
 ...
 * ============================================================================
 */ 
//# sourceMappingURL=creator.abstract.js.map