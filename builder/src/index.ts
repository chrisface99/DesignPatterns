...
import {
  ...
  ComputerDirector,
} from "./01-basic-concept/computer-builder";

import {
  SelectQueryBuilder,
  InsertQueryBuilder,
} from "./02-advanced-example/sql-query-builder";
...
function runBasicExample(): void {
  ...
  const budgetGaming = director.buildBudgetGamingPC(new GamingComputerBuilder());
  ...
  const minimalPC = director.buildMinimalOfficePC(new OfficeComputerBuilder());
  ...
  try {
    new GamingComputerBuilder().setCPU("i5").setRAM(8).getResult();
  } catch (e) {
  ...
  try {
    new OfficeComputerBuilder().setCPU("i3").setRAM(4).getResult();
  } catch (e) {
    console.log(`   ❌ ${(e as Error).message}`);
  }
...
function runAdvancedExample(): void {
  ...
  const simpleQuery = new SelectQueryBuilder()
    .select("name", "email")
    .from("users")
    .build();
  ...
  const filteredQuery = new SelectQueryBuilder()
  ...
  const complexQuery = new SelectQueryBuilder()
  ...
  try {
    new SelectQueryBuilder().select("*").build();
  } catch (e) {
    console.log(`   ❌ ${(e as Error).message}`);
  }
