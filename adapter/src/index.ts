import { USPlug, EUSocket, UKSocket, EUToUSAdapter, UKToUSAdapter, USDevice } from "./01-basic-concept/plug-adapter";
import { PaymentProcessor, StripeSDK, PayPalSDK, StripeAdapter, PayPalAdapter, PaymentService } from "./02-advanced-example/payment-adapter";
...
└──────────────────┘
  ...
  const euAdapter = new EUToUSAdapter(euSocket);
  const usDevice = new USDevice(euAdapter);
  console.log(`\n📌 ${usDevice.powerOn()}`);

  // 1C: With UK adapter
  sub("1C: Another adapter — UK to US");

  const ukSocket = new UKSocket();
  const ukAdapter = new UKToUSAdapter(ukSocket);
  const usDevice2 = new USDevice(ukAdapter);
  ...
  const devices = [
    new USDevice(new EUToUSAdapter(new EUSocket())),
    new USDevice(new UKToUSAdapter(new UKSocket())),
  ];
...
function runAdvancedExample(): void {
  ...
  const stripeProcessor: PaymentProcessor = new StripeAdapter(new StripeSDK());
  ...
  const paypalProcessor: PaymentProcessor = new PayPalAdapter(new PayPalSDK());
  ...
  function processWith(provider: PaymentProcessor, name: string): void {
  ...
  processWith(new StripeAdapter(new StripeSDK()), "Stripe");
  processWith(new PayPalAdapter(new PayPalSDK()), "PayPal");
