import { Logger, EagerSingleton, ModernSingleton } from "./01-basic-concept/logger-singleton";
import { ConfigManager, DatabaseConnection } from "./02-advanced-example/config-and-db";
...
function runBasicExample(): void {
  ...
  const modern1 = ModernSingleton.instance;
  const modern2 = ModernSingleton.instance;
...
function runAdvancedExample(): void {
  ...
  console.log(`📌 db.port: ${config.get<number>("db.port")}`);
  ...
  const dbPort = config.get<number>("db.port");
